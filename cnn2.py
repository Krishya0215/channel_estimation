...
def test_model(input_audio_path, output_audio_path, model, device, sample_rate=48000, duration=15):
    ...
    reconstructed_spectrum = np.concatenate([np.zeros(cutoff_index), reconstructed_spectrum[cutoff_index:]])
...
best_loss = float('inf')
csv_filename = 'training_metrics_cnn2.csv'
...
if __name__ == '__main__':
    ...
    with open(csv_filename, mode='w', newline='') as csv_file:
        ...
        for epoch in range(num_epochs):
            ...
            print(f'cnn2, Epoch [{epoch + 1}/{num_epochs}], Loss: {avg_loss:.4f}')
    ...
    output_audio_path = 'predict/test_output_cnn2.wav'
    model.load_state_dict(torch.load('best_model_cnn2.pth', map_location=device))
