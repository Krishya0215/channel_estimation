...
best_loss = float('inf')
csv_filename = 'training_metrics_cnn_continue4.csv'
...
def continue_training(model, optimizer, data_loader, device, start_epoch, num_epochs, lr, csv_filename, best_loss):
    # 加载已保存的模型权重
    model.load_state_dict(torch.load('best_model_cnn.pth', map_location=device))
    optimizer.param_groups[0]['r'] = lr  # 更新学习率
    L = [(signal_batch, double_batch) for signal_batch, double_batch in data_loader]

    with open(csv_filename, mode='a', newline='') as csv_file:  # 以追加模式写入
        csv_writer = csv.writer(csv_file)
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            print(f'cnn_continue4, Epoch [{epoch + 1}/{num_epochs}], Loss: {avg_loss:.4f}')
            csv_writer.writerow([epoch + 1, avg_loss])

            if avg_loss < best_loss:
                best_loss = avg_loss
                torch.save(model.state_dict(), 'best_model_cnn_continue4.pth')
...
if __name__ == '__main__':
    ...
    start_epoch = 56408
    lr = 0.0001
    continue_training(model, optimizer, data_loader, device, start_epoch, num_epochs, lr, csv_filename, best_loss)

    # 测试模型
    input_audio_path = 'music/test_input.wav'
    output_audio_path = 'predict/test_output_cnn.wav'
    model.load_state_dict(torch.load('best_model_cnn_continue4.pth', map_location=device))
    test_model(input_audio_path, output_audio_path, model, device, sample_rate=sample_rate, duration=duration)
    gc.collect()
